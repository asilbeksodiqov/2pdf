...
function removeImage(index) {
    ...
    if (imageManager.getCount() === 0) {
        toast.show('O`chirildi!', 'Rasm olib tashlandi', 'info');
    } else {
        toast.show('O`chirildi!', 'Rasm olib tashlandi', 'info');
    }
}
...
function removeAllImages() {
    ...
    updatePreview();
    toast.show('O`chirildi!', 'Barcha rasmlar olib tashlandi', 'info');
}
...
async function sharePDF() {
    ...
    try {
        ...
        await navigator.share({
            ...
            text: `sodiqov.uz/2pdf bilan tayyorlandi.`
        });
...
async function handleFeedbackSubmit(e) {
    ...
    if (!feedbackManager.canSendFeedback()) {
        ...
        toast.show('Kuting!', `${minutes} daqiqadan keyin xabar yuborishingiz mumkin`, 'warning');
        return;
    }
...
<b>Reyting:</b> ${rating}/5
        ...
        if (offlineManager && !offlineManager.checkConnection()) {
            ...
            if (sent) {
                ...
                toast.show('Muvaffaqiyatli!', 'Fikringiz uchun tashakkur', 'success');
